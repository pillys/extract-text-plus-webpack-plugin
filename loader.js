...
module.exports.pitch = function(request) {
  ...
  if (this[__dirname] === undefined) {
    throw new Error(
      '"extract-text-plus-webpack-plugin" loader is used without the corresponding plugin, ' +
    ...
    var resultSource;
    if (query.remove) {
      resultSource = '// removed by extract-text-plus-webpack-plugin';
    } else {
      resultSource = undefined;
    }
    if (query.extract !== false) {
      var childFilename = path.basename(this.resourcePath) || 'extract-text-plus-webpack-plugin-output-filename'; // eslint-disable-line no-path-concat
      var publicPath = typeof query.publicPath === 'string' ? query.publicPath : this._compilation.outputOptions.publicPath;
      var outputOptions = {
        filename: childFilename,
        publicPath: publicPath
      };
      var childCompiler = this._compilation.createChildCompiler('extract-text-plus-webpack-plugin', outputOptions);
      childCompiler.apply(new NodeTemplatePlugin(outputOptions));
      childCompiler.apply(new LibraryTemplatePlugin(null, 'commonjs2'));
      childCompiler.apply(new NodeTargetPlugin());
      childCompiler.apply(new SingleEntryPlugin(this.context, '!!' + request));
      childCompiler.apply(new LimitChunkCountPlugin({
        maxChunks: 1
      }));
