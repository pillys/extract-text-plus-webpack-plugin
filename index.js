...
function ExtractTextPlusPluginCompilation() {
  this.modulesByIdentifier = {};
}

ExtractTextPlusPlugin.prototype.mergeNonInitialChunks = function(chunk, intoChunk, checkedChunks) {
  if (!intoChunk) {
    checkedChunks = [];
...
ExtractTextPlusPluginCompilation.prototype.addModule = function(identifier, originalModule, source, additionalInformation, sourceMap, prevModules) {
  var m;
...
ExtractTextPlusPluginCompilation.prototype.addResultToChunk = function(identifier, result, originalModule, extractedChunk) {
...
ExtractTextPlusPlugin.prototype.renderExtractedChunk = function(chunk) {
...
function ExtractTextPlusPlugin(id, filename, options) {
...
module.exports = ExtractTextPlusPlugin;
...
ExtractTextPlusPlugin.loader = function(options) {
  return require.resolve('./loader') + (options ? '?' + JSON.stringify(options) : '');
};

ExtractTextPlusPlugin.extract = function(before, loader, options) {
  if (typeof loader === 'string' || Array.isArray(loader)) {
    if (typeof before === 'string') {
      before = before.split('!');
    }
    return [
      ExtractTextPlusPlugin.loader(mergeOptions({
        omit: before.length,
        extract: true,
        remove: true
      }, options))
...
ExtractTextPlusPlugin.prototype.applyAdditionalInformation = function(source, info) {
  if (info) {
...
ExtractTextPlusPlugin.prototype.loader = function(options) {
  options = JSON.parse(JSON.stringify(options || {}));
  options.id = this.id;
  return ExtractTextPlusPlugin.loader(options);
};

ExtractTextPlusPlugin.prototype.extract = function(before, loader, options) {
  if (typeof loader === 'string' || Array.isArray(loader)) {
    if (typeof before === 'string') {
      before = before.split('!');
    }
    return [
      this.loader(mergeOptions({
        omit: before.length,
        extract: true,
        remove: true
      }, options))
...
ExtractTextPlusPlugin.prototype.apply = function(compiler) {
  var options = this.options;
  compiler.plugin('this-compilation', function(compilation) {
    var extractCompilation = new ExtractTextPlusPluginCompilation();
    ...
    compilation.plugin('additional-assets', function(callback) {
      ...
      extractedChunks.forEach(function(extractedChunk) {
        if (extractedChunk.modules.length) {
          ...
          if (typeof options.nameFilter === 'function') {
            file = options.nameFilter(file);
          }
          if (options.allEntries) {
            concatCss.push(source.source());
          } else {
      ...
      if (options.allEntries) {
        concatSource = new ConcatSource(concatCss.join('\n\n'));
